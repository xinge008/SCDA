import torch
from extensions._nms._ext import nms
import numpy as np
...
def pth_nms(dets, thresh):
  ...
  #  return keep[:num_out[0]]
    ...
    #x2 = dets[:, 2]
    #y2 = dets[:, 3]
    # scores = dets[:, 4].cuda().contiguous()
    dets = dets.cuda().contiguous()


    #areas = (x2 - x1 + 1) * (y2 - y1 + 1)
    # order = scores.sort(0, descending=True)[1][:6000]
    ...
    nms.gpu_nms(keep, num_out, dets.float(), thresh)

    return keep[:num_out[0]].cpu().contiguous()
